...
export default function NavMenu() {
  ...
  const activeCheck = (nav: NavType) => {
    return segment && nav.path.includes(segment);
  };
  return (
    <>
      {NAVS.map((nav) => (
        <li key={nav.name}>
          <Link href={`/${nav.path[0]}`}>
            <div className={style.navPill}>
              <>
                ...
                >
                  <g>
                    <path
                      d={activeCheck(nav) ? nav.svgPath1 : nav.svgPath2}
                    ></path>
                  </g>
                </svg>
                <div style={activeCheck(nav) ? { fontWeight: "bold" } : {}}>
                  {nav.name}
                </div>
              </>
            </div>
